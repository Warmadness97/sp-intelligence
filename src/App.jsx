...
import {
  ...
  TrendingDown, Minus, Loader2, Info, Pencil
} from "lucide-react";
import {
...
const MARKET_DATA_GROUPS = [
  ...
  { key: "economic", label: "Economic", items: [["cpi", "CPI"], ["ppi", "PPI"], ["pmi", "PMI"], ["gdp", "GDP"], ["nfp", "NFP"]] },
  { key: "market", label: "Market", items: [["vix", "VIX"], ["move", "MOVE"], ["fearGreed", "Fear & Greed"], ["dxy", "DXY"], ["bondYield", "Bond Yield"]] },
  { key: "equity", label: "Equity", items: [["sp500", "S&P 500"], ["nasdaq", "Nasdaq"], ["dow", "Dow"], ["russell", "Russell"], ["sox", "SOX"]] },
  { key: "sector", label: "Sector", items: [["ai", "AI"], ["semiconductor", "Semiconductor"], ["ev", "EV"], ["financial", "Financial"], ["energy", "Energy"]] },
];
...
export default function SPIntelligencePlatform() {
  ...
  const generateReport = async (force = false) => {
    ...
    try {
      ...
      const dataFieldsSpec = MARKET_DATA_GROUPS
...
alerts 請針對距離 Knock-In 過近（如小於 10%）或到期集中、標的集中等情況提出具體警示。`;
      ...
      setAiError(e?.message ? `AI 報告產生失敗：${e.message}` : "AI 報告產生失敗，請稍後再試一次。");
    } finally {
      setAiLoading(false);
    }
  };
...
function Dashboard({ computed, dimScores, weights, holdings, band, reports, generateReport, aiLoading, aiError }) {
  ...
  const latestReport = reports[0];
  const generatedToday = latestReport && isToday(latestReport.timestamp);
      ...
      {showForm && (
        <Panel className="p-5">
          ...
          <div className="flex gap-2 mt-4">
            ...
            {editingId && (
              <button onClick={cancelForm} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ backgroundColor: "#1B222E", color: "#8B94A6" }}>
                取消編輯
              </button>
            )}
          </div>
          ...
          return (
            <Panel key={h.id} className="p-4">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => startEdit(h)} style={{ color: "#8B94A6" }} title="編輯">
                    <Pencil size={15} />
                  </button>
                  <button onClick={() => removeHolding(h.id)} style={{ color: "#4A5568" }} title="刪除">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
